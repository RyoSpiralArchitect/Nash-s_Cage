...
\begin{provenancebox}
The operator identifies the v0.1 source and PDF as the preserved upload. The v0.1 source hash is independently present in earlier repository payload history; the v0.1 PDF has no earlier in-repository historical anchor, so its upload identity remains operator-attested. The operator likewise identifies this v0.2 as a regenerated revision dated 7 August 2026, based on the v0.1 source and extended with the executable-companion contract below. Historical v0.2 bytes are unavailable, and byte identity with any earlier v0.2 is not claimed. The same-tree release manifest records current file identities and checks internal consistency; it does not independently prove lineage or authenticity. A separately reviewed Git commit hash, signed tag, or release signature can provide an external anchor when distributed through an independent channel.
\end{provenancebox}
...
\midrule
F0 & Executable structural toy & The declared mechanisms compose into a coherent, inspectable, replayable loop under recorded conditions. & Deterministic tests, explicit assumptions, traceable outputs, hash-addressed receipts, and a reviewed external anchor for authenticity claims; the same-tree manifest checks internal consistency only. \\
\addlinespace
...
The current manifest and receipt system detects missing or byte-modified declared files relative to metadata committed in the same tree. It is not a signature, transparency-log inclusion proof, independent build receipt, or proof that the prose attached to a hash is historically true. Provenance statements in this release are therefore operator-attested except where repository history supplies an independent earlier anchor.
...
$\Gamma$ & \texttt{select\_mode()}, \texttt{schedule\_policy()}, \texttt{advance\_policy()} & Requested trigger mode, pending transition and delay, eventual active mode, and release logic. The trace exposes \texttt{requested\_mode}, \texttt{active\_mode}, \texttt{pending\_mode}, their names, and \texttt{pending\_delay} separately. \\
...
\begin{enumerate}
  ...
  \item \textbf{Observable outputs.} Every complete run writes \path{summary.csv}, \path{episodes.csv}, \path{trace.csv}, \path{comparison.md}, \path{resolved_config.json}, and \path{receipt.json}. The receipt records the claim boundary, command, declared environment, source identity, configuration identity, and output hashes. The trace records requested, pending, and active modes in distinct columns. A request may remain pending while the active mode and its policy targets are delayed.
  \item \textbf{Verification.} Receipt verification resolves recorded paths relative to the receipt, recomputes SHA-256 identities, and returns non-zero for a missing or modified file. Repository verification also checks same-tree manifest consistency and the committed reference receipt, then replays the declared reference command and byte-compares its deterministic outputs. Successful replay establishes byte consistency under the exercised source and configuration; it does not establish semantic correctness, historical provenance, or authenticity.
  \item \textbf{Failure visibility.} False-positive and false-negative requested-trigger classifications are reported together. \texttt{emergency\_request\_rate} counts steps on which the trigger requests emergency-or-higher mode; it does not measure time spent in an active emergency mode. Skipped executable tests, absent release artifacts, and hash mismatches are failures, not successful partial results. Manuscript buildability is qualified separately with an explicit TeX toolchain and must not be inferred from Python CI.
\end{enumerate}
...
\begin{boundarybox}
\textbf{What F0 does not establish.} The normalized parameters are selected to expose an inspectable mechanism gradient. They do not estimate climate sensitivity, tipping distributions, institutional response, actor preferences, or real intervention effects. The executable establishes correspondence, byte replayability under the recorded source, configuration, seed, and compatible runtime, and falsifiable interfaces---not the correctness of mode semantics, historical provenance, authenticity, empirical truth, policy rank, safety, legality, or democratic legitimacy.
\end{boundarybox}
...
5. Select requested mode:
   if rho_t < rho_min:
       requested_mode <- loss minimization
   else if lower_bound(CR_t) <= 0:
       requested_mode <- emergency braking
   else if CR_t is low or boundary uncertainty is high:
       requested_mode <- precaution
   else:
       requested_mode <- adaptive normal
...
7. Schedule and advance actuators:
   pending_mode, pending_delay <- schedule(requested_mode)
   active_mode <- advance pending transition when its delay elapses
   apply price, cap, permit, finance, restoration, dividend, compensation
   according to active_mode

8. Audit adaptation:
   search for metric, boundary, delay, narrative, finance, and justice hacks

9. Publish:
   observations, models, uncertainty, requested/pending/active mode,
   burdens, beneficiaries, override and release conditions
